...
public class PeriodicIntervalTrigger extends Trigger {
    ...
    public void start(Plugin plugin) {
        ...
        task = Bukkit.getScheduler().runTaskTimer(plugin, () -> {
            tickCounter += 20;
            // Нулевая нагрузка: если ни один скрипт не слушает core:interval, ничего не создаем
            if (!kostin.ak.actionstriggers.api.ActionTriggerAPI.getTriggers().hasDirectSubscriptions(KEY)) {
                return;
            }
