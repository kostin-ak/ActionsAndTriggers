    ...
    public void render(@NotNull GuiContext ctx, @NotNull Map<Integer, ItemStack> matrix) {
        ...
        if (meta != null) {
            ...
            if (entry.getName() != null) {
                meta.displayName(mm.deserialize(kostin.ak.actionstriggers.api.hook.PapiHook.parse(ctx.getPlayer(), entry.getName())));
            }
            if (entry.getLore() != null && !entry.getLore().isEmpty()) {
                List<Component> loreList = new ArrayList<>();
                for (String l : entry.getLore()) {
                    loreList.add(mm.deserialize(kostin.ak.actionstriggers.api.hook.PapiHook.parse(ctx.getPlayer(), l)));
                }
                meta.lore(loreList);
            }
