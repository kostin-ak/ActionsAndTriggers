...
- **Поддерживает кастомные предметы Oraxen и ItemsAdder**: в полях `material`, `icon`, `empty_item` можно указывать `oraxen:item_id` или `itemsadder:item_id`. Движок автоматически вызывает `ActionTriggerAPI.getItems().resolveItem(...)`.
- **Имеет прямые встроенные колбеки экшенов**:
  - `on_click: [ - action: ... ]` — выполняется при клике на кнопку.
  - `on_insert: [ - action: ... ]` — выполняется при помещении предмета в слот.
  - `on_extract: [ - action: ... ]` — выполняется при извлечении предмета.
  - `on_change: [ - action: ... ]` — выполняется при смене состояния переключателя.
  - `on_open: [ - action: ... ]` — выполняется при открытии инвентаря игроком.
  - `on_close: [ - action: ... ]` — выполняется при закрытии инвентаря игроком.
  *Вам не нужно писать отдельные триггеры для каждого клика со стороны — все действия настраиваются прямо внутри виджета!*
