        ...
        if (placeholderMaterial != null && !placeholderMaterial.isEmpty()) {
            ...
            if (meta != null) {
                ...
                if (placeholderName != null && !placeholderName.isEmpty()) {
                    meta.displayName(MiniMessage.miniMessage().deserialize(kostin.ak.actionstriggers.api.hook.PapiHook.parse(ctx.getPlayer(), placeholderName)));
                }
                if (placeholderLore != null && !placeholderLore.isEmpty()) {
                    List<Component> lore = new ArrayList<>();
                    for (String l : placeholderLore) {
                        lore.add(MiniMessage.miniMessage().deserialize(kostin.ak.actionstriggers.api.hook.PapiHook.parse(ctx.getPlayer(), l)));
                    }
                    meta.lore(lore);
                }
