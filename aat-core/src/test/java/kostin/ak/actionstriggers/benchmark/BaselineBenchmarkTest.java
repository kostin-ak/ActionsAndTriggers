...
public class BaselineBenchmarkTest {
    ...
    public void runBaselineBenchmarks() throws IOException {
        ...
        Path targetDocs = Files.exists(Path.of("..", "docs")) ? Path.of("..", "docs") : Path.of("docs");
        if (!Files.exists(targetDocs)) {
            Files.createDirectories(targetDocs);
        }
        File reportFile = targetDocs.resolve("BENCHMARK_REPORT.md").toFile();
